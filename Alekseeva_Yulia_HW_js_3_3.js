...
//actions
const getDepartments = function (compains) {
compains.forEach((company) => {
    let depts = [];
    let count = 0;
    depts.push(company.name);
    if(compains.departments) {
        compains.departments.forEach(dept => {
...
const deleteDepartament = function(val) {
...
// deleteDepartament(10)
// console.log(enterprises[2])

//Написать функцию для переноса сотрудников между отделами одного предприятия.
...
const moveEmployess = function(currentId, newId) {
